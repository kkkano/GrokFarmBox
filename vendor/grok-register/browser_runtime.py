...
def create_browser_options(browser_proxy="", extension_path=None):
    ...
    if _config.get("browser_silent"):
        # 静默：窗口移到主屏外，不弹到桌面视野；仍为有头浏览器，保留过 Turnstile/风控能力
        options.set_argument("--window-position=-2400,-2400")
        options.set_argument("--window-size=1280,820")
